...
- Applies semantic validation beyond JSON Schema for IDs and ordering
- Strips HTML comments and redundant whitespace for token efficiency
- Prunes completed roadmap tasks into archive.log
- Emits .ai/LATEST_CONTEXT.md with metadata, session ID, and validation summary
"""
...
class SchemaEngine:
    ...
    def validate(self, file_name: str, payload: dict[str, Any], file_path: Path) -> None:
        ...
        if not schema_name:
            raise MemoryValidationError(f"{file_path}: no schema registered for file")
    ...
    def _validate_semantics(self, file_name: str, payload: dict[str, Any], file_path: Path) -> None:
        if file_name == "01_global_context.md":
            naming = payload.get("naming", {})
            for key in ["task_id_regex", "decision_id_regex", "risk_id_regex", "blocker_id_regex"]:
                regex_value = naming.get(key, "")
                try:
                    re.compile(regex_value)
                except re.error as exc:
                    raise MemoryValidationError(
                        f"{file_path}: naming.{key} is not a valid regex: {exc}"
                    ) from exc
        ...
        if file_name == "02_roadmap.md":
            ...
            for index, task in enumerate(tasks, start=1):
                ...
                for dep in task.get("depends_on", []):
                    if dep == task_id:
                        raise MemoryValidationError(
                            f"{file_path}: task '{task_id}' cannot depend on itself"
                        )
                    if dep not in all_ids:
                        raise MemoryValidationError(
                            f"{file_path}: task '{task_id}' depends on missing task '{dep}'"
                        )

        if file_name == "03_logic_map.md":
            component_names = [component["name"] for component in payload.get("components", [])]
            if len(component_names) != len(set(component_names)):
                raise MemoryValidationError(
                    f"{file_path}: component names must be unique"
                )
        ...
        if file_name == "04_active_buffer.md":
            ...
            if orders != expected_orders:
                raise MemoryValidationError(
                    f"{file_path}: next_actions.order must be contiguous starting at 1"
                )

            for key in ("risks", "blockers"):
                ids = [item["id"] for item in payload.get(key, [])]
                if len(ids) != len(set(ids)):
                    raise MemoryValidationError(
                        f"{file_path}: {key} ids must be unique"
                    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--session-id", default=os.getenv("SESSION_ID", ""), help="Stable identifier for the current chat or workflow session.")
...
def prune_completed_tasks(roadmap_path: Path, archive_file: Path, session_id: str, schema_engine: SchemaEngine) -> PruneResult:
...
def main() -> int:
    ...
    for path in sorted(memory_root.rglob("*.md")):
        if path.name.startswith("."):
            continue
        files.append(parse_memory_file(path=path, schema_engine=schema_engine))

    validation_summary = ValidationSummary(
        files_validated=len(files) + 1,
